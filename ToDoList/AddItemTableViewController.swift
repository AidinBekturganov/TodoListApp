...
class ItemDetailViewController: UITableViewController {
    ...
    @IBOutlet weak var textFieldType: UITextView!
    ...
    @IBAction func doneButton(_ sender: Any) {
        if let item = itemToEdit, let text = textFieldType.text{
            item.text = text
            flag = false
            delegate?.itemDetailViewController(self, didFinishEditingItem: item, flag: flag)
        }else{
            if let item = todoList?.newToDo(){
                if let textFieldText = textFieldType.text{
...
extension ItemDetailViewController: UITextViewDelegate{
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        guard let oldText = textFieldType.text,
